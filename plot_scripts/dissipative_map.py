...
df = pd.read_csv('../rust/eigenvalues.csv', header=None)
...
real_cols = df.columns[2::2]
imag_cols = df.columns[3::2]
...
point, = ax.plot(initial_data['x_norm'].values, 
...
def update(val):
    ...
    if not row_data.empty:
        point.set_data(row_data['x_norm'].values, row_data['y_norm'].values)
