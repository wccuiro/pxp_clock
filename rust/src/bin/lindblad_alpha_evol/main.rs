                ...
                for site in 0..l {
                    ...
                    for ii in 0..l {
                        for jj in 0..l {
                            ...
                            let c4 = (state_in.states_a[ii] & (1u64 << (site % l))) == 0;
                            if c1 && c4 {
                                // FIXED: Squaring the state-dependent factors to correctly compute L^\dagger L
                                let f1 = calc_factor(state_in.states_a[ii], site + l - 1);
                                let f2 = calc_factor(state_in.states_a[ii], site + 1);
                                let p_factor = (f1 * f1) * (f2 * f2);
                    ...
                    for ii in 0..l {
                        for jj in 0..l {
                            ...
                            if c1 && c4 {
                                // FIXED: Squaring the state-dependent factors to correctly compute L^\dagger L
                                let f1 = calc_factor(state_out.states_b[ii], site + l - 1);
                                let f2 = calc_factor(state_out.states_b[ii], site + 1);
                                let p_factor = (f1 * f1) * (f2 * f2);
...
// HELPER: Efficient Sparse Matrix-Vector product to avoid loading O(N^2) memory.
fn sparse_dot(mat: &CsMat<Complex64>, vec: &Array1<Complex64>) -> Array1<Complex64> {
    let mut result = Array1::zeros(vec.len());
...
fn obs_evolution(
    ...
    lindbladian: &CsMat<Complex64>, // FIXED: Take sparse matrix directly
    neel_indices: &[usize], 
    dt: f64,
    t_final: f64,
) -> Array1<(f64, f64, f64)> {
    ...
    for _step in 0..=num_steps {
        ...
        // FIXED: 4th-Order Runge-Kutta instead of Euler for stability.
        let k1 = sparse_dot(lindbladian, &current_rho);
        let rho_k2 = &current_rho + &(&k1 * (dt / 2.0));
        
        let k2 = sparse_dot(lindbladian, &rho_k2);
        let rho_k3 = &current_rho + &(&k2 * (dt / 2.0));
        
        let k3 = sparse_dot(lindbladian, &rho_k3);
        let rho_k4 = &current_rho + &(&k3 * dt);
        
        let k4 = sparse_dot(lindbladian, &rho_k4);

        current_rho = &current_rho + &((&k1 + &(&k2 * 2.0) + &(&k3 * 2.0) + &k4) * (dt / 6.0));
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let results: Vec<String> = parameters.par_iter()
        .map(|&(gp, gm, omega, alpha)| {
            ...
            // FIXED: Passing the sparse matrix directly to obs_evolution, destroying the memory-hogging dense conversion
            let l_cal_0 = build_lindbladian(l, &basis_0, 0, omega, gamma_plus, gamma_minus, alpha);
            let obs_0 = obs_evolution(l, &n_mat_0, &corr_mat_0, &basis_0, 0, &rho_0, &l_cal_0, &neel_idx_0, dt, t_final);

            let l_cal_pi = build_lindbladian(l, &basis_pi, q_pi, omega, gamma_plus, gamma_minus, alpha);
            let obs_pi = obs_evolution(l, &n_mat_pi, &corr_mat_pi, &basis_pi, q_pi, &rho_pi, &l_cal_pi, &neel_idx_pi, dt, t_final);
